...
<div class="flex-1 flex flex-col bg-background h-full overflow-hidden">
	...
	{:else}
		...
		{#if error}
			<div
				class="bg-destructive/10 border-l-4 border-destructive px-4 py-3 flex items-center justify-between flex-shrink-0"
			>
		...
		{#if hasNoCommits && !error}
			<div
				class="bg-blue-500/10 border-l-4 border-blue-500 px-4 py-3 flex items-center gap-2 flex-shrink-0"
			>
		...
		{:else if hasNoRemote && !hasNoCommits && !error}
			<div
				class="bg-yellow-500/10 border-l-4 border-yellow-500 px-4 py-3 flex items-center justify-between flex-shrink-0"
			>
				<div class="flex items-center gap-2">
		...
		<!-- Tabs and Diff Pane Container -->
		<div class="flex-1 flex overflow-hidden">
			<div class="flex-1 flex flex-col min-h-0 overflow-hidden">
				<Tabs.Root
					value={activeTab}
					onValueChange={(v) => (activeTab = (v as typeof activeTab) || 'changes')}
					class="flex-1 flex flex-col min-h-0"
				>
					<div class="px-4 border-b flex-shrink-0">
													...
													<ScrollArea class="h-[400px]">
														...
														>
															{#each unstagedFiles as file, index}
																...
																<div
																	class="flex items-center justify-between p-3 rounded-lg border transition-colors cursor-pointer {isSelected
			...
			{#if selectedFileForDiff && fileDiff}
				<div
					class="w-1/2 border-l border-border flex flex-col overflow-hidden cursor-pointer"
					onclick={closeDiffPane}
					role="button"
					tabindex="0"
				>
					<div
						class="p-3 border-b border-border flex items-center justify-between bg-muted/30 flex-shrink-0"
					>
						<div class="flex items-center gap-2">
							...
							<div class="font-mono text-xs">
								{#each fileDiff.lines as line}
									{#if line.line_type === 'hunk'}
										<div
											class="bg-muted/50 px-2 py-1 text-muted-foreground border-y border-border sticky top-0"
										>
											{line.content}
										</div>
									{:else}
										<div
											class="flex hover:bg-muted/30 {line.line_type === 'add'
										...
										>
											<!-- Old line number -->
											<div
												class="w-12 flex-shrink-0 text-right px-2 py-0.5 select-none border-r border-border/50 text-muted-foreground/60 {line.line_type ===
												'add'
											...
											<!-- New line number -->
											<div
												class="w-12 flex-shrink-0 text-right px-2 py-0.5 select-none border-r border-border text-muted-foreground/60 {line.line_type ===
												'add'
													? 'bg-green-100 dark:bg-green-950/50'
													: line.line_type === 'delete'
														? 'bg-red-100 dark:bg-red-950/50'
											...
											<!-- Content -->
											<div
												class="flex-1 px-3 py-0.5 whitespace-pre-wrap break-all {line.line_type ===
												'add'
		...
		{#if stagedFiles.length > 0}
			<div class="border-t border-border bg-muted/20 p-3 flex-shrink-0">
				<div class="flex gap-2 items-end">
					...
					<Button onclick={handleCommit} disabled={!commitMessage.trim()} class="px-4">
					...
					<Button variant="ghost" size="icon" class="flex-shrink-0" title="More commit options">
	...
	>
		<Card.Root class="w-full max-w-md" onclick={(e) => e.stopPropagation()}>
			...
			<Card.Footer class="flex gap-2">
				<Button variant="outline" class="flex-1" onclick={() => (showCommitDialog = false)}>
					Cancel
				</Button>
				<Button class="flex-1" onclick={handleCommit} disabled={!commitMessage.trim()}>
					Commit
				</Button>
			</Card.Footer>
		</Card.Root>
	...
	>
		<Card.Root class="w-full max-w-md" onclick={(e) => e.stopPropagation()}>
			<Card.Header>
				...
				<Card.Description>Configure a remote repository to push/pull changes</Card.Description>
			</Card.Header>
			<Card.Content>
				<div class="space-y-4">
					<div>
			...
			<Card.Footer class="flex gap-2">
				<Button variant="outline" class="flex-1" onclick={() => (showRemoteDialog = false)}>
					Cancel
				</Button>
				<Button
					class="flex-1"
		...
		<Card.Root class="w-full max-w-md" onclick={(e) => e.stopPropagation()}>
			...
			<Card.Content>
				<div class="space-y-4">
					...
					<div>
						<label for="git-password" class="text-sm font-medium mb-2 block"
			...
			<Card.Footer class="flex gap-2">
				<Button variant="outline" class="flex-1" onclick={() => (showCredentialsDialog = false)}>
					Cancel
				</Button>
				<Button
					class="flex-1"
